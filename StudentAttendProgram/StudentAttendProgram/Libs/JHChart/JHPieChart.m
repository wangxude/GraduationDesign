...
-(void)drawRect:(CGRect)rect{
    ...
    if (_descArr.count==_valueArr.count&&_descArr.count>0) {
        ...
        for (NSInteger i = 0; i<_descArr.count; i++) {
             ...
             [self drawQuartWithColor:colors[i%colors.count] andBeginPoint:P_M(15+self.frame.size.width/2*(i%1), 20*(i/1  )+25+_chartArcLength*2) andContext:contex];
            
            CGFloat present = [_valueArr[i] floatValue]/_allValueCount*100;
            [self drawText:[NSString stringWithFormat:@"%@ 人数:% 3ld 占比:%.1f%c",_descArr[i],[_valueArr[i] integerValue],present,'%'] andContext:contex atPoint:P_M(30+self.frame.size.width/2*(i%1), 20*(i/1  )+25+_chartArcLength*2) WithColor:[UIColor blackColor] andTextFontSize:14];
        }
...
-(void)showAnimation{
    ...
    if (_valueArr.count>0) {
        ...
        if (_descArr.count>0) {
            ...
            NSInteger i = _descArr.count/1 + _descArr.count%1;
    ...
    for (NSInteger i = 0; i<_countPreAngeleArr.count-1; i++) {
        ...
        if ([_countPreAngeleArr[i+1] floatValue]>=angel) {
            ...
            if (_descArr.count>0) {
                ...
                NSInteger i = _descArr.count/1 + _descArr.count%2;
            ...
            [UIView animateWithDuration:0.3 animations:^{
                if (_saveIndex==i) {
                    ...
                    if (saveItems.center.x==self.frame.size.width/2) {
                        ...
                        itemsView.center = CGPointMake(self.frame.size.width/2+xSpa, 21+wid/2+spa);
...
//                        _showInfoView.frame = C   GRectMake(p.x, p.y, _showInfoView.frame.size.width, _showInfoView.frame.size.height);
                        ...
                        _showInfoView.hidden = YES;
                        saveItems.center = CGPointMake(self.frame.size.width/2, 21+wid/2);
