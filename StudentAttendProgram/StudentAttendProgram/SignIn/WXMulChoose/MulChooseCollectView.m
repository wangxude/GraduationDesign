...
-(void)CreateCollectView{
    ...
    _MyCollectView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 0 ,ScreenWidth, self.frame.size.height) collectionViewLayout:flowLayout];
...
-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    return CGSizeMake((ScreenWidth-40)/3,ItemHeight);
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.iconImageView.image = [UIImage imageNamed:[_iconDataArray objectAtIndex:indexPath.row]];
    ...
    if (self.firstSelect == 0) {
        if ([_choosedArr containsObject:cell.titleLab.text]) {
